...
export const COMPONENT_REGISTRY: ComponentEntry[] = [
...

  {
    ...
    category: "canvas-feature",
    description:
      "Automated multi-node processing pipeline with reliability features. When a node completes, downstream nodes auto-execute with 500ms delay. Supports execution locks, error propagation, retry, per-node timeouts, progress tracking, and chain cancel.",
    props: [],
    hooks: [
      { name: "useRef", source: "react", purpose: "Execution locks, abort controller, running state" },
      { name: "useCallback", source: "react", purpose: "Memoized execute, abort, retry functions" },
      { name: "useState", source: "react", purpose: "Reactive chain progress state" },
    ],
    capabilities: [
      ...
      "Error state propagation: failed nodes mark downstream as 'blocked' (I2)",
      "retryNode(nodeId): clear error and resume chain from failed node (I3)",
      "ChainProgress tracking: totalNodes, completedNodes, currentNodeLabel (I4)",
      "Per-node timeout: configurable by node type via NODE_TIMEOUTS map (I5)",
      "abortChain() with AbortController and cancelled status propagation (I6)",

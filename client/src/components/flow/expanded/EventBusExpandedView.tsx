 ...
 *
 * Configures mode (publish/listen), channel name, and shows the event log.
 */
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="p-4 border-b border-border/50">
            <h3 className="text-sm font-semibold">
              {mode === "publish" ? "Agent Connection Guide" : "Incoming Results"}
            </h3>
          </div>
              ...
              {mode === "publish" ? (
                <div className="space-y-2 font-mono text-[11px] text-muted-foreground">
                  ...
                  <p className="font-sans text-[10px] text-muted-foreground mt-2">
                    All requests require <code>X-API-Key</code> header. Set <code>PROVOCATIONS_API_KEY</code> and <code>PROVOCATIONS_USER_ID</code> on the server.
                  </p>
                ...
                <ScrollArea className="h-48 rounded-md border border-border">
                  <div className="p-2 space-y-1">
                    {[...log].reverse().map((entry) => (
                      <div key={entry.id} className="text-[11px] flex gap-2 py-1.5 border-b border-border/50 last:border-0">
                        <span className="text-muted-foreground shrink-0 w-16">
                          {new Date(entry.timestamp).toLocaleTimeString()}
                        </span>
                        <span className={`shrink-0 w-12 ${entry.type === "task" ? "text-cyan-500" : "text-emerald-500"}`}>
                          {entry.type}
                        </span>
